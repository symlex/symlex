<template>
    <div class="md-table-pagination">
        ...
        <md-select v-model="currentSize" md-menu-class="md-pagination-select" @change="changeSize" v-if="mdPageOptions">
            <md-option v-for="(amount, index) in mdPageOptions" :value="amount" :key="index">{{ amount }}</md-option>
        </md-select>
    ...
    export default {
        ...
        mounted() {
            this.$nextTick(() => {
                ...
                this.currentSize = this.currentSize ? this.currentSize : this.mdPageOptions[0];
